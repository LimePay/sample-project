(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["LimePayASModule"] = factory();
	else
		root["LimePayASModule"] = factory();
})(window, function() {
...
const FormValidator = __webpack_require__(33);
const ConfigValidator = __webpack_require__(34);
const thirdPartyPaymentProvider = __webpack_require__(10);
const thirdPartyPaymentProviderConfig = __webpack_require__(35);

let InitializationService = (function () {

    let isFormInitialized = false;

    class InitializationService {

        static async init (limePayToken, config) {
            if (!this.isValidLimePayToken(limePayToken)) {
                throw new Error(constants.ERRORS.INVALID_LIME_PAY_TOKEN);
            }
            ConfigValidator.validate(config);
            FormValidator.validate();

            this.limePayToken = limePayToken;
            this.paymentToken = await _retrieveBSToken(limePayToken, config.URL);
        ...
        static isInitializedCorrect () {
            return isFormInitialized;
        }

        static isValidLimePayToken (limePayToken) {
            if (limePayToken) {
                let tokenParts = limePayToken.split('.');
    ...
    let _buildThirdPartyConfigFromLimeConfig = function (limeConfig) {
        limeConfig.isFailedSubmit = false;
        Object.assign(limeConfig, thirdPartyPaymentProviderConfig);

        let onErrorFn = limeConfig.onFieldEventHandler.onError;
        limeConfig.onFieldEventHandler.onError = function (tagId, errorCode) {
            onErrorFn(tagId, errorCode);
            if (limeConfig.isFailedSubmit) {
                limeConfig.isFailedSubmit = false;
                limeConfig.eventHandler.onFailedSubmit({ message: 'Invalid data in a hosted field' });
            }
        }
    }

    let _preventActionPaymentForm = function () {
    ...
    let _retrieveBSToken = async function (limePayToken, BASE_URL) {
...
module.exports = {
    ERRORS: {
        INVALID_LIME_PAY_TOKEN: "Invalid limepaytoken",
        NO_TRANSACTIONS_PROVIDED: "No transactions provided!",
        INCORRECT_FORM_INITIALIZATION: "Form is not initialized properly",
        INCORRECT_ARRAY_OF_SIGNED_TRANSACTIONS: "Result from sign callback in not array of signed transactions",
        NO_SIGNED_TRANSACTIONS: "It should have at least one signed transaction.",
        INVALID_TX_FORMAT: (tx) => {
            return "Transaction has been provided with invalid format. TX: " + JSON.stringify(tx);
        }
    },
    URLs: {
...
"use strict";
/* eslint-disable */  !function () { if ("function" == typeof NodeList.prototype.forEach) return; NodeList.prototype.forEach = Array.prototype.forEach }(); var bluesnap = function () { var a, e, t, n, i = !0, r = "https://includestest.ccdc02.com/cardinalcruise/v1/songbird.js", o = "https://sandbox.bluesnap.com/services/hosted-payment-fields/v2.0/bluesnap.iframe.html#", s = { token: "", style: {}, ccnPlaceHolder: "Card number", cvvPlaceHolder: "CVC/CVV", expPlaceHolder: "MM/YY", expDropDownSelector: !1 }, c = { hostedPaymentFields: {}, onFieldEventHandler: {} }, l = { "3DS": !1 }, d = { "3DS": {} }, u = [], p = !1, v = !1, f = "", h = { cvv: !1, ccn: !1, exp: !1, responseJSON: {}, setSuccess: function (e, t) { if (this[e] = !0, "ccn" === e) { var n = JSON.parse(t); this.responseJSON.ccType = n.ccType, this.responseJSON.last4Digits = n.last4Digits, this.responseJSON.issuingCountry = "Planet Earth"; try { null != n.issuingCountry && (this.responseJSON.issuingCountry = n.issuingCountry) } catch (e) { w("JSON error, " + e) } } "exp" === e && (this.responseJSON.exp = t), this.cvv && this.ccn && this.exp && this.callback && (w("All three iFrames updated successfully"), !0 === l["3DS"] && 0 < Object.keys(d["3DS"]).length ? D({ tagId: "ccn", event: "authenticateWith3DS", threeDSecureFields: d["3DS"], jwt: f }) : this.callback(this.responseJSON)) }, setCallBack: function (e) { this.ccn = u.indexOf("ccn") < 0, this.cvv = u.indexOf("cvv") < 0, this.exp = u.indexOf("exp") < 0, this.callback = e }, set3DSAuthenticationResult: function (e) { var t = {}; t.authResult = e, this.responseJSON.threeDSecure = t, this.callback(this.responseJSON) } }; function m() { try { this.ccn = y("ccn"), this.cvv = y("cvv"), this.exp = y("exp") } catch (e) { console.log("Missing IFrame id, " + e) } } function y(e) { return c.hostedPaymentFields.hasOwnProperty(e) ? document.querySelector("[data-bluesnap=" + c.hostedPaymentFields[e] + "]") : null } function g(e) { var t = new m, n = { ccn: t.ccn, cvv: t.cvv, exp: t.exp }; try { return n[e] } catch (e) { console.log("this is not a valid id, " + e) } } function S() { var i = ""; try { "_" !== s.token.slice(-1) && (o = "https://www" + s.token.slice(-1) + ".bluesnap.com/services/hosted-payment-fields/v2.0/bluesnap.iframe.html#"), i = o } catch (e) { w("cannot retreive domain, " + e) } try { u.forEach(function (e) { var t, n, a; g(e).innerHTML = (t = e, n = i, a = document.location.origin, "<iframe frameborder='0' style='border: medium none; width: 100%; height: 100%; float: left;' src='" + (n += window.btoa(a + t + ("exp" === t && s.expDropDownSelector ? "_" : ""))) + "' allowtransparency='true' scrolling='no' type=" + t + " name='bluesnap-hosted-iframe-" + t + "' id='bluesnap-hosted-iframe-" + t + "'></iframe>"), "exp" === e && document.querySelector("#bluesnap-hosted-iframe-exp").setAttribute("bluesnapselect", s.expDropDownSelector) }) } catch (e) { w("cannot create iframes, " + e) } } function b(e) { if ("put" === e.event) w("Received success response for Put request from iFrame [" + e.tagId + "]"), h.setSuccess(e.tagId, e.param); else try { var t = "on" + ((n = e.event).charAt(0).toUpperCase() + n.slice(1)); switch (t) { case "onError": c.onFieldEventHandler[t](e.tagId, e.errorCode, e.errorDescription); break; case "onType": c.onFieldEventHandler[t](e.tagId, e.cardType); break; default: c.onFieldEventHandler[t](e.tagId) } } catch (e) { console.log("onFieldEventHandler is not valid, " + e) } var n } function D(e) { g(e.tagId).querySelectorAll("iframe[id^='bluesnap-hosted-iframe-']")[0].contentWindow.postMessage(JSON.stringify(e), o.split("/services/")[0]) } function w(e) { !1 === i && console.log("BS.Main: " + e) } function x() { var e, t, n; "_" !== s.token.slice(-1) && (r = "https://includes.ccdc02.com/cardinalcruise/v1/songbird.js"), w("Loading cardinalcruise js"), e = r, t = function () { var e = "off"; !1 === i && (e = "on"), Cardinal.configure({ logging: { level: e } }), Cardinal.on("payments.setupComplete", function () { w("payments.setupComplete callback was called"), null != a && (w("Enabling the submit button"), a.disabled = !1) }), Cardinal.on("payments.validated", function (e, t) { var n; w("payments.validated callback was called"), w(JSON.stringify(e)), n = t, w("Posting processAuthenticationResult message to iFrame[ccn]"), D({ tagId: "ccn", event: "processAuthenticationResult", jwt: f, resultJwt: n }) }) }, (n = document.createElement("script")).type = "text/javascript", n.src = e, n.onreadystatechange = t, n.onload = t, document.head.appendChild(n) } return e = window, t = "message", n = function (e) { if (e.origin === o.split("/services/")[0]) { var t, n = ""; if (e.data) try { n = JSON.parse(e.data) } catch (e) { w("JSON parse error, " + e) } if ("implementation" === n.event) w("Received implementation message from iFrame [" + n.tagId + "]"), w("Posting implementation message back to iFrame [" + n.tagId + "]"), D({ tagId: n.tagId, event: "implementation", tokenAndStyle: s }), !0 === l["3DS"] && !1 === v && !1 === p && (w("Jwt wasn't created nor sent. Posting createJwt message to iFrame [" + n.tagId + "]"), p = !0, D({ tagId: n.tagId, event: "createJwt" })); else if ("cardinalSetup" === n.event) w("Received cardinalSetup message from internal.js. Setting jwtCreated = true"), v = !0, f = n.jwt, t = f, w("Calling Cardinal.setup"), Cardinal.setup("init", { jwt: t }); else if ("cardinalContinue" === n.event) { w("Received cardinalContinue message from internal.js"); var a = n.enrollmentData, i = a.enrollmentStatus; "Yes" === i ? function (e) { w("Calling Cardinal.continue"); var t = { AcsUrl: e.acsUrl, Payload: e.payload }, n = { OrderDetails: { TransactionId: e.transactionId } }; Cardinal.continue("cca", t, n) }(a) : (w("enrollmentStatus = " + i), h.set3DSAuthenticationResult(i)) } else "authenticationResult" === n.event ? (w("Received authenticationResult message from internal.js"), h.set3DSAuthenticationResult(n.authResult)) : b(n) } }, e.addEventListener ? e.addEventListener(t, n, !1) : e.attachEvent ? e.attachEvent("on" + t, n) : e["on" + t] = n, { submitCredentials: function (e, t) { h.setCallBack(e); var n, a = function (e) { if (!0 === l["3DS"]) { var t = e.amount, n = e.currency, a = ""; if ("number" != typeof t && "string" != typeof n ? a = " { amount, currency }" : "number" != typeof t ? a = " { amount }" : "string" != typeof n && (a = " { currency }"), "" !== a) return { tagId: "ccn", event: "error", errorCode: "14102", errorDescription: "3D Secure object is missing required fields" + a }; d["3DS"] = e } }(t); null == a ? (w("Posting " + (n = "put") + " message to iFrames"), u.forEach(function (e) { D({ tagId: e, event: n }) })) : b(a) }, hostedPaymentFieldsCreation: function (e, n) { u = [], n.hostedPaymentFields = {}, document.querySelectorAll("[data-bluesnap]").forEach(function (e) { var t = e.getAttribute("data-bluesnap"); "submitButton" === t ? a = e : "ccn" !== t && "cvv" !== t && "exp" !== t || (u[u.length] = t, n.hostedPaymentFields[t] = t) }); var t = function (e) { var t = { styles: ["color", "font", "font-family", "font-size", "font-style", "font-weight", "line-height", "opacity", "outline", "text-shadow", "transition", "left"], fields: ["#ccn", "#ccn:focus", "#ccn.valid", "#ccn.invalid", "#cvv", "#cvv:focus", "#cvv.valid", "#cvv.invalid", "#year", "#year:focus", "#year.valid", "#year.invalid", "#month", "#month:focus", "#month.valid", "#month.invalid", "input", "::placeholder", "::-webkit-input-placeholder", "::-moz-placeholder", ":-ms-input-placeholder", ":-moz-placeholder", ":focus", ".valid", ".invalid", ".valid:focus", ".invalid:focus", "span", "select"] }; if ("object" == typeof e.style && null !== e.style) { for (var n in e.style) { if (-1 === t.fields.indexOf(n)) return [!1, "error"]; for (var a in e.style[n]) if (-1 === t.styles.indexOf(a)) return [!1, "error"] } return [!0, "object"] } return [!0, "not an object"] }(n); t[0] ? ("object" === t[1] ? s.style = n.style : delete s.style, s.token = e, void 0 !== n.ccnPlaceHolder && null !== n.ccnPlaceHolder && (s.ccnPlaceHolder = n.ccnPlaceHolder), void 0 !== n.cvvPlaceHolder && null !== n.cvvPlaceHolder && (s.cvvPlaceHolder = n.cvvPlaceHolder), void 0 !== n.expPlaceHolder && null !== n.expPlaceHolder && (s.expPlaceHolder = n.expPlaceHolder), void 0 !== n.expDropDownSelector && null !== n.expDropDownSelector && (s.expDropDownSelector = n.expDropDownSelector), void 0 !== n["3DS"] && null !== n["3DS"] && !0 === n["3DS"] && (l["3DS"] = !0, null != a && (a.disabled = !0), x()), c.hostedPaymentFields = n.hostedPaymentFields, c.onFieldEventHandler = n.onFieldEventHandler, S()) : console.log(t[1] + " check style did not pass") } } }(); module.exports = bluesnap;

/***/ }),
/* 11 */
/***/ (function(module, exports, __webpack_require__) {

const LimePayWeb = __webpack_require__(12);
const HOST = __webpack_require__(43).HOST;
const countries = __webpack_require__(44);
...
const LimePayWeb = __webpack_require__(13);
module.exports = LimePayWeb;
...
const InitializationService = __webpack_require__(2);
const PaymentServiceInstance = __webpack_require__(36);
const TransactionsBuilder = __webpack_require__(38);
const utils = __webpack_require__(41);

let main = {
    init: async function (limePayToken, config) {
        await InitializationService.init(limePayToken, config);
...
class FormValidator {
    static validate () {
        return FormValidator.hasCheckoutForm() &&
            FormValidator.hasExpDate() &&
            FormValidator.hasCVV() &&
            FormValidator.hasCardNumber();
    }

    static hasCheckoutForm () {
        let checkoutForm = document.getElementById('checkout-form');
        if (!checkoutForm && checkoutForm.tagName != 'FORM') {
            throw new Error("Document element with id [checkout-form] is not defined");
        }
    }
    ...
    static hasExpDate () {
        ...
        if (!expDate && expDate.getAttribute('data-bluesnap') != 'exp') {
            throw new Error("Document element with id [exp-date] is not defined");
        }
    }
    ...
    static hasCVV () {
        ...
        if (!cvv && cvv.getAttribute('data-bluesnap') != 'cvv') {
            throw new Error("Document element with id [cvv] is not defined");
        }
    }

    static hasCardNumber () {
        let cardNumber = document.getElementById('card-number');
        if (!cardNumber && cardNumber.getAttribute('data-bluesnap') != 'ccn') {
            throw new Error("Document element with id [card-number] is not defined");
        }
    }
}
...
class ConfigValidator {
    ...
    static validate (config) {
        ...
        if (!config.URL || typeof config.URL !== "string") {
            throw new Error('URL is not defined or it is not a string');
        }
        ...
        if (config.eventHandler) {
            ...
            if (!config.eventHandler.onSuccessfulSubmit || !(config.eventHandler.onSuccessfulSubmit instanceof Function)) {
                throw new Error('eventHandler.onSuccessfulSubmit is not defined or it is not a function');
            }
            ...
            if (!config.eventHandler.onFailedSubmit || !(config.eventHandler.onFailedSubmit instanceof Function)) {
                throw new Error('eventHandler.onFailedSubmit is not defined or it is not a function');
            }
        } else {
...
/* eslint-disable */
let BlueSnapCheckoutConfiguration = (function () {
    ...
    let configuration = {
        onFieldEventHandler: {
            ...
            onError: function (tagId, errorCode) {
                ...
                document.getElementById(`${tagId}-help`).textContent = getErrorText(errorCode);
            },
            ...
            onValid: function (tagId) {
                ...
                document.getElementById(`${tagId}-help`).textContent = "";
            }
        },
        style: {
            // Styling all inputs
    ...
    function changeImpactedElement(tagId, removeClass, addClass) {
        ...
        addClass = addClass || "";
        let impactedElement = document.querySelector(`[data-bluesnap=${tagId}]`);
...
const InitializationService = __webpack_require__(2);
const thirdPartyPaymentProvider = __webpack_require__(10);
const CardHolderInformationValidator = __webpack_require__(37);

let wrappedPaymentService = (function () {
    ...
    class PaymentService {
        ...
        async processPayment (cardHolderInformation, signedTransactions) {
            if (!InitializationService.isInitializedCorrect()) {
                throw new Error(constants.ERRORS.INCORRECT_FORM_INITIALIZATION);
            }

            let validationErrors = CardHolderInformationValidator.validate(cardHolderInformation);
            if (validationErrors) {
                configuration.eventHandler.onFailedSubmit({ message: JSON.stringify(validationErrors) });
            }

            if (!Array.isArray(signedTransactions)) {
                throw new Error(constants.ERRORS.INCORRECT_ARRAY_OF_SIGNED_TRANSACTIONS);
            } else if (signedTransactions.length == 0) {
                throw new Error(constants.ERRORS.NO_SIGNED_TRANSACTIONS);
            }

            configuration.isFailedSubmit = true;
            thirdPartyPaymentProvider.submitCredentials(async function (cardData) {
                try {
                    ...
                    configuration.eventHandler.onFailedSubmit({ message: err.message });
                }
            });
        }
    }

    let _createPayment = async function (authTokens, cardHolderInformation, signedTransactions) {
        ...
        let request = {
            url: `${configuration.URL}/v1/checkout`,
            method: "POST",
            headers: {
                "Content-Type": "application/json",
        ...
        try {
            ...
            configuration.eventHandler.onFailedSubmit(e.response);
        }
    }
...
class CardHolderInformationValidator {

    static validate (cardHolderInformation) {
        let validationErrors = [];

        if (!cardHolderInformation || cardHolderInformation.constructor.name !== 'Object') {
            validationErrors.push('CardHolderInformation is not an object');
        }

        if (!cardHolderInformation.name || typeof cardHolderInformation.name !== "string") {
            validationErrors.push('CardHolderInformation.name is not a string');
        }

        if (!!cardHolderInformation.isCompany || typeof cardHolderInformation.isCompany !== "boolean") {
            validationErrors.push('CardHolderInformation.isCompany is not a boolean');
        }

        if (!cardHolderInformation.vatNumber || typeof cardHolderInformation.vatNumber !== "string") {
            validationErrors.push('CardHolderInformation.vatNumber is not a string');
        }

        if (!cardHolderInformation.countryCode || typeof cardHolderInformation.countryCode !== "string") {
            validationErrors.push('CardHolderInformation.address.countryCode is not a string');
        }

        if (!cardHolderInformation.zip || typeof cardHolderInformation.zip !== "string") {
            validationErrors.push('CardHolderInformation.address.zip is not a string');
        }

        if (!cardHolderInformation.street || typeof cardHolderInformation.street !== "string") {
            validationErrors.push('CardHolderInformation.address.street is not a string');
        }
    }
}
...
const axios = __webpack_require__(1);
const ethers = __webpack_require__(39);
const constants = __webpack_require__(4);

const InitializationService = __webpack_require__(2);

let wrappedTransactionBuilder = (function () {
    ...
    class TransactionsBuilder {
        ...
        async buildSignedTransactions (jsonWallet, password, transactions) {
            if (!transactions) {
                throw new Error(constants.ERRORS.NO_TRANSACTIONS_PROVIDED);
            }

            var wallet = await initWallet(jsonWallet, password);
            let request = {
                url: `${BASE_URL}${constants.URLs.GAS_PRICE_NONCE_PATH}` + wallet.address,
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
            ...
            let nonceCounter = response.data.nonce;
            let gasPrice = ethers.utils.bigNumberify(response.data.gasPrice);

            let signedTransactions = [];
            for (let i = 0; i < transactions.length; i++) {
                if (!isValidTx(transactions[i])) {
                    throw new Error(constants.ERRORS.INVALID_TX_FORMAT(transactions[i]));
                }

                let txOptions = { gasLimit: transactions[i].gasLimit, gasPrice: gasPrice, value: transactions[i].value, nonce: nonceCounter++ }
                let contractInstance = new ethers.Contract(transactions[i].to, transactions[i].abi, wallet);
                let functionParameters = transactions[i].functionParams || [];
                let signedTx = await createSignedTransaction(contractInstance, transactions[i].functionName, wallet, txOptions, transactions[i].to, functionParameters);
    ...
    function isValidTx (tx) {
        ...
        return (
            ...
            tx.to &&
            tx.abi &&
            Number.isInteger(tx.gasLimit) &&
            Number.isInteger(tx.value) &&
            tx.functionName
        );
    }

    function createSignedTransaction (contract, functiontToExecute, wallet, transactionOptions, to, functionParams) {
        let currentFunc = contract.interface.functions[`${functiontToExecute}`];

        const encodedFunc = currentFunc.encode(functionParams);
    ...
    async function initWallet (json, password) {
...
const CardHolderVATDataValidator = __webpack_require__(42);
const axios = __webpack_require__(1);

let utils = {
    initCalculateVAT: function (limePayToken, envURL) {

        let envUrl = envURL;
        let token = limePayToken;

        return async function (cardHolderVATData) {
            CardHolderVATDataValidator.validate(cardHolderVATData);

            let request = {
                url: `${envUrl}/v1/checkout/vat`,
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "x-lime-token": token,
...
class CardHolderVATDataValidator {

    static validate (cardHolderVATData) {
        if (!cardHolderVATData || cardHolderVATData.constructor.name !== 'Object') {
            throw new Error('vatObject is not an object');
        }

        if (!cardHolderVATData.countryCode || typeof cardHolderVATData.countryCode !== "string" || cardHolderVATData.countryCode.length != 2) {
            throw new Error('countryCode is not defined or it is not with appropriate length');
        }
        if (typeof cardHolderVATData.isCompany !== "boolean") {
            throw new Error('isCompany is not defined or it is not a boolean');
        }
...
module.exports = {
    ...
    // HOST: 'http://test-limepay-api.eu-west-1.elasticbeanstalk.com'
}
