...
const ConfigValidator = __webpack_require__(32);
const thirdPartyPaymentProvider = __webpack_require__(10);
const thirdPartyPaymentProviderConfig = __webpack_require__(33);
...
module.exports = {
    ERRORS: {
        ...
        INVALID_TX_FORMAT: (tx) => {
            return "Transaction has been provided with invalid format. TX: " + tx.toString()
        }
    },
    URLs: {
...
const InitializationService = __webpack_require__(1);
const PaymentServiceInstance = __webpack_require__(34);
const TransactionsBuilder = __webpack_require__(36);
...
let utils = {
    validateForm: function () {
        return utils.hasSubmitButton() &&
            utils.hasCheckoutForm() &&
            utils.hasExpDate() &&
            utils.hasCVV() &&
            utils.hasCardNumber();
    },
    ...
    hasSubmitButton: function () {
        ...
        if (!submitButton || submitButton.tagName != 'BUTTON') {
            throw new Error("Document element with id [submit-button] is not defined");
        }
    },

    hasCheckoutForm: function () {
        let checkoutForm = document.getElementById('checkout-form');
...
class ConfigValidator {
    ...
    static validate (config) {
        ...
        // Validate that signingTXCallback is defined
        if (!config.signingTxCallback || !(config.signingTxCallback instanceof Function)) {
            throw new Error('signingTxCallback callback is not a function');
        }
...
const InitializationService = __webpack_require__(1);
const thirdPartyPaymentProvider = __webpack_require__(10);
const CardHolderInformationValidator = __webpack_require__(35);
...
let wrappedPaymentService = (function () {
    ...
    class PaymentService {
        ...
        async processPayment (cardHolderInformation) {
            ...
            thirdPartyPaymentProvider.submitCredentials(async function (cardData) {
                try {
                    ...
                    let signedTransactions = await execTxSigning();
    ...
    let execTxSigning = async function () {
        try {
            var signedTransactions = await configuration.signingTxCallback();
...
class CardHolderInformationValidator {
    ...
    static validate (cardHolderInformation) {
        ...
        if (!cardHolderInformation.vatNumber || typeof cardHolderInformation.vatNumber !== "string") {
            validationErrors.push('CardHolderInformation.vat_number is not a string');
        }

        if (!cardHolderInformation.countryCode || typeof cardHolderInformation.countryCode !== "string") {
            validationErrors.push('CardHolderInformation.address.country_code is not a string');
        }

        if (!cardHolderInformation.zip || typeof cardHolderInformation.zip !== "string") {
            validationErrors.push('CardHolderInformation.address.postal_zip_code is not a string');
        }
...
const axios = __webpack_require__(2);
const ethers = __webpack_require__(37);
const constants = __webpack_require__(4);
    ...
    class TransactionsBuilder {
        ...
        async buildSignedTransactions (jsonWallet, password, transactions) {
            ...
            for (let i = 0; i < transactions.length; i++) {
                ...
                let signedTx = await createSignedTransaction(contractInstance, transactions[i].fnName, wallet, txOptions, transactions[i].to, transactions[i].params);
